...
public class CuttingCounter : BaseCounter, IHasProgress
{
    ...
    [SerializeField]
    private CuttingReciepeSO[] cutKitchenObjectOSArray;
        ...
        if (!HasKitchenObject())
        {
            ...
            if (player.HasKitchenObject())
            {
                ...
                if (HasRecipeWithInput(player.GetKitchenObject().GetKitchenObjectOS()))
                {
                    ...
                    CuttingReciepeSO cuttingReciepeSO = GetCuttingSOWithInput(
                    ...
                    OnProgressChanged?.Invoke(
                        ...
                        {
                            progressNormalized =
                                (float)cuttingProgress / cuttingReciepeSO.cuttingPogressMax,
                        }
                    );
                }
            }
    ...
    public override void InteractAlternate(Player player)
    {
        if (HasKitchenObject() && HasRecipeWithInput(GetKitchenObject().GetKitchenObjectOS()))
        {
            ...
            {
                KitchenObjectOS outputKitchenObjectSO = GetOutputForInput(
                    GetKitchenObject().GetKitchenObjectOS()
                );
